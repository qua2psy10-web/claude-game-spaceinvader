...
| 難易度の選択 | ← → | EASY/NORMAL/HARD ボタン |
| サウンド ON/OFF | M | 🔊 ボタン(右上) |

### 難易度

タイトル画面で **EASY / NORMAL / HARD** を選べます(選択は保存され次回に復元)。
難易度で変わるのは **敵の速さ** と **敵の攻撃頻度** の2つで、残機(3機)とスコアは共通です。
...
- Web Audio API — 効果音と BGM をオシレーターでリアルタイム合成(外部音声ファイル不使用)
  - BGM はホルスト「火星」風の 5/4 拍子オスティナート + 半音階で軋む旋律の 4 小節ループ。敵が減る・ウェーブが進むほどテンポアップ
